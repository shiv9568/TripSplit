...
import { ArrowLeft, Sparkles, Receipt, IndianRupee, Tag, Check, Users, MessageSquare, ArrowRight } from 'lucide-react';
...
export default function AddExpense() {
  ...
  const fetchTrip = async () => {
    try {
      ...
      if (data.members.length > 0) setPayer(data.members[0]);
    } catch {
                     ...
                     <div className="flex gap-3 overflow-x-auto pb-4 no-scrollbar -mx-2 px-2">
                        {CATEGORIES.map(cat => (
                           <button
                              ...
                              className={`flex-shrink-0 px-5 py-3 rounded-2xl border-2 flex items-center gap-2 font-black text-xs uppercase tracking-tight transition-all ${
                                 category === cat.value 
                                 ? 'bg-[#0B1A2C] border-[#0B1A2C] text-white shadow-lg' 
                  ...
                  <div className="space-y-4">
                     ...
                     <div className="grid grid-cols-2 gap-3">
                        {members.map(member => (
                           ...
                           >
                              {member}
                              {payer === member && <Check size={16} strokeWidth={3} />}
                           </button>
                        ))}
                     </div>
                  </div>
